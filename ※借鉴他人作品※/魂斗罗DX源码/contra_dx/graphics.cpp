...
void Render32bpp_Normal( unsigned char* lpRdr, unsigned char* pdest, unsigned int pitch);
void Render16bpp_Normal( unsigned char* lpRdr, unsigned char* pdest, unsigned int pitch);

void (*blitter) ( unsigned char* lpRdr, unsigned char* pdest, unsigned int pitch);

int framerate=60;
int Crt_BPP, xres, yres;
...
int initVedeo(int fullscreen){
	...
	xres =dmode.dmPelsWidth;
	yres =dmode.dmPelsHeight;
	if (freq<57){
		if (fherr)
...
int	i, j;
	...
	get_loss_shift(&Rloss, &Rsft, &Gloss, &Gsft, &Bloss, &Bsft);
...
void nesBlit(unsigned char* pdest, unsigned int pitch){
	blitter(&Renderbuf[8], pdest, pitch);
}

void	Render32bpp_Normal( unsigned char* lpRdr, unsigned char* pdest, unsigned int pitch)
{
	unsigned char*	pScn = lpRdr;
	unsigned char*	pDst = pdest;
	unsigned char*	pPal;
