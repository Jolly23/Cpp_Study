...
int initgame(HINSTANCE hInst, int nCmdShow){
	int wstyle;
	...
	//	20	奖命声
	//	21	飞机开动声
	//	24	三向弹落地爆炸声 /自爆桥爆炸声 /手雷爆炸声 /落地弹爆炸声 /钉板爆炸声
	//	25	里关小关爆炸声 /岛爆炸声
	//	26	标题音乐
	//	46	过关音乐
	//	4A	ENDING音乐
	...
	//	52	损命声
	//	54	暂停声
	//	55	战车爆炸声 /BOSS 5爆炸声 /BOSS 6爆炸声 /魔鬼头爆炸声
	//	57	BOSS爆炸声
}
...
long PASCAL WinProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	switch(message){
		case WM_CREATE:
			...
			SetRect(&r, 0, 0, SCREEN_W*2, SCREEN_H*2);
			AdjustWindowRectEx(&r, GetWindowLong(hWnd, GWL_STYLE), 0, GetWindowLong(hWnd, GWL_EXSTYLE));
			if (!fullscreen)
				SetWindowPos(hWnd, HWND_TOPMOST, 30, 60, r.right - r.left, r.bottom - r.top, SWP_SHOWWINDOW);
			else
...
void game_sub0(){
	...
	unsigned char* pvars;
	int i;
	static int *pbgno =(bossbgm==1) ?bgno2:bgno;
